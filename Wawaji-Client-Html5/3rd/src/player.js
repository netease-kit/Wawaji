...
var Player = function(url, options) {
	...
	if (url instanceof Array) {
		this.source = new JSMpeg.Source.WhiteBoards(url, options);
		options.streaming = true;
	} else if (typeof url === 'object') {
		this.source = new JSMpeg.Source.WhiteBoard(url, options);
		options.streaming = true;
	}
...
Player.prototype.stopRender = function () {
	if (this.source && this.source.stopRender instanceof Function) {
		this.source.stopRender()
	}
	if (this.video) {
		this.video.setIgnorePFrame(true)
	}
	// this.renderer.canvas.style.visibility = 'hidden'
	this.renderer.canvas.style.zIndex = 0
}

Player.prototype.startRender = function () {
	if (this.source.startRender instanceof Function) {
		this.source.startRender()
	}
	// this.renderer.canvas.style.visibility = 'visible'
	this.renderer.canvas.style.zIndex = 1
}

Player.prototype.setStreamId = function (id) {
	if (this.source.setStreamId instanceof Function) {
		this.source.setStreamId(id)
	}
}
