...
class ChatRoomAction {
  ...
  __getChatroomAddressDone (error, obj) {
    ...
    let self = this
    const {address} = obj
    window._chatroom = SDK.Chatroom.getInstance({
  ...
  giveUpForGame () {
    let self = this
    window._chatroom.queuePoll({
      elementKey: this._account,
      done (err, obj, content) {
        if (err && err.code === 'Error_Connection_Socket_State_not_Match') {
          window.alert(`由于网络问题，放弃排队失败`)
          return
        }
        self.inQueue = false
