...
void VChat::StartDeviceEx(const std::string& id, const std::string& device_path, int width, int height, nim_vchat_start_device_cb_func cb)
{
	std::string json_value;
	if (width * height > 0)
	{
	...
	NIM_SDK_GET_FUNC(nim_vchat_start_extend_camera)(id.c_str(), device_path.c_str(), 25, json_value.c_str(), cb, nullptr);
}
void VChat::EndDeviceEx(const std::string& id)
{
	NIM_SDK_GET_FUNC(nim_vchat_stop_extend_camera)(id.c_str(), "");
}
