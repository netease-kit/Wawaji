...
void nim_delete_array(void** ptr)
{
	if (ptr && *ptr) {
		delete [](*ptr);
		*ptr = nullptr;
...
#define CC_SAFE_FREE(ptr) (void)nim_free((void**)(&ptr));
#define CC_SAFE_DELETE(ptr)(void)nim_delete((void**)(&ptr));
#define CC_SAFE_DELETE_ARRAY(ptr) (void)nim_delete_array((void**)(&ptr));
...
{
	nbase::ThreadManager::PostTask(kThreadSerialOpt, cb);
                     ...
                     const std::vector<std::string>* protocols/* = nullptr*/)
{
	...
	if (lws_parse_uri((char*)url.c_str(), &prot, &i.address, &i.port, &p))
	{
		//error
		return false;
	}
...
//            "permessage-deflate; client_no_context_takeover; client_max_window_bits"
    ...
    //info.options = 0;
    info.user = this;
	info.client_ssl_ca_filepath = _caPath.c_str();
	info.ws_ping_pong_interval = 0;
	info.options |= LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT;
    int log_level = LLL_ERR | LLL_WARN | LLL_NOTICE | LLL_HEADER | LLL_EXT | LLL_CLIENT | LLL_LATENCY| LLL_INFO | LLL_DEBUG | LLL_PARSER;/*| LLL_INFO | LLL_DEBUG | LLL_PARSER*/
    ...
    if (nullptr != _wsContext)
    {
        ...
        std::string ads_port = _host + ":" + portStr;
		 lws_client_connect_info _connectInfo;
		 memset(&_connectInfo, 0, sizeof _connectInfo);
		_connectInfo.context = _wsContext;
		_connectInfo.ssl_connection = _useSsl;
		_connectInfo.host = _host.c_str();
		_connectInfo.origin = _host.c_str();
		_connectInfo.protocol = name.c_str();
		_connectInfo.path = _path.c_str();
		_connectInfo.address = _host.c_str();
		_connectInfo.port =_port;
		_connectInfo.ietf_version_or_minus_one = -1;
		_wsInstance = lws_client_connect_via_info(&_connectInfo);
...
void WebSocket::onClientWritable()
{
    ...
    if (iter != _wsHelper->_subThreadWsMessageQueue->end())
    {
        ...
        const ssize_t n = min(remaining, c_bufferSize);
...
void WebSocket::onClientReceivedData(void* in, size_t len)
{
    // In websocket thread
