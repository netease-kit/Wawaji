...
/** @struct RtsStatusCbInfo
  * @brief 白板通话中的监听
  */
struct RtsStatusCbInfo
{
	nim::Rts::ConnectNotifyCallback connect_cb_;
	nim::Rts::MemberNotifyCallback member_cb_;
	nim::Rts::RecDataCallback rec_data_cb_;
};
/** @class RtsManager
* @brief 白板管理器
...
public:
	...
	* @return void 无返回值
	*/
	void AddRtsStatusCb(const std::string& session_id, RtsStatusCbInfo info);
	/**
	* 移除白板会话监听
	* @param[in] session_id 会话id
	* @return void 无返回值
	*/
	void RemoveRtsStatusCb(const std::string& session_id);

private:
	WindowEx* GetRtsFromBySessionId(const std::string& session_id);
	RtsStatusCbInfo *GetRtsStatusCbBySessionId(const std::string& session_id);
	std::map<std::string, RtsStatusCbInfo> map_rts_cb_list_;
};
}