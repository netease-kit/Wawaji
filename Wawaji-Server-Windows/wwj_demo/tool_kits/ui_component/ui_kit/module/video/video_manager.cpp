...
void VideoManager::OnVChatEvent(nim::NIMVideoChatSessionType type, uint64_t channel_id, int code, const std::string& json)
{
	...
	case nim::kNIMVideoChatSessionTypeStartRes:{
		...
		else if (chatroom_start_cb_)
		{
			chatroom_start_cb_(code);
		}
		else
		{
	...
	case nim::kNIMVideoChatSessionTypeCalleeAckNotify:{
		...
		if (reader.parse(json, valus))
		{
			...
			{
				chatroom_start_cb_(accept ? 200 : 0);
			}
		}
	}break;
...
{
	int32_t webrtc = atoi(GetConfigValue("webrtc").c_str());
	std::string json_value;
	if (webrtc == 1)
	{
		Json::FastWriter fs;
...
bool VideoManager::JoinRoom(nim::NIMVideoChatMode mode, const std::string& room_name, const std::string& rtmp_url, bool live_link, const std::string& session_id, nim::VChat::Opt2Callback cb)
{
	Json::FastWriter fs;
	Json::Value value;
	value[nim::kNIMVChatSessionId] = session_id;
	...
	value[nim::kNIMVChatRtmpRecord] = atoi(GetConfigValue("rtmp_record").c_str());
	value[nim::kNIMVChatAudioHighRate] = 1;
	std::string split_mode = GetConfigValue("split_mode");
	if (split_mode.empty())
	{
...
void VideoManager::SetChatRoomCb(ConnectCallback start_cb, ConnectCallback connect_cb /*= nullptr*/, PeopleChangeCallback people_cb /*= nullptr*/, VolumeCallback volume_cb /*= nullptr*/)
{
	chatroom_start_cb_ = start_cb;
