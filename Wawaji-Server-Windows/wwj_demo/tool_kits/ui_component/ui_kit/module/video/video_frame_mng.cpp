...
void VideoFrameMng::AddVideoFrame(bool capture, int64_t time, const char* data, int size, int width, int height, const std::string& json, FrameType frame_type)
{
	...
	if (reader.parse(json, valus))
	{
		...
		std::string id = valus[nim::kNIMDeviceId].asString();
		if (!id.empty())
		{
			account = id;
			capture = false;
	...
	//{
	//	int32_t dest_w = min(width, height);
	//	int32_t dest_h = min(width, height);
	//	int32_t w_start = (width - dest_w) / 2;
	//	int32_t h_start = (height - dest_h) / 2;
	//	w_start -= w_start % 2;
	//	h_start -= h_start % 2;
	//	ret_data.resize(dest_w*dest_h * 3 / 2);
	//	uint8_t* des_y = (uint8_t*)ret_data.c_str();
	//	uint8_t* des_u = des_y + dest_w * dest_h;
	//	uint8_t* des_v = des_u + dest_w * dest_h / 4;
	//	libyuv::ConvertToI420((const uint8_t*)src_buffer, size,
	//						  des_y, dest_w,
	//						  des_u, dest_w / 2,
	//						  des_v, dest_w / 2,
	//						  w_start, h_start,
