		...
		if (msg.msg_setting_.need_offline_ != nim::BS_TRUE)//只在线
		{
			if (msg.type_ == nim::kNIMSysMsgTypeCustomP2PMsg)
			{
				...
				if (StringToJson(msg.attach_, json) && json.isObject())
				{
					...
					else if (json["command"].isInt())
					{
						nim_chatroom::MeetingFrontpage* meeting_form = static_cast<nim_chatroom::MeetingFrontpage*>(WindowsManager::GetInstance()->GetWindow(nim_chatroom::MeetingFrontpage::kClassName, nim_chatroom::MeetingFrontpage::kClassName));
						if (meeting_form != NULL)
						{
							meeting_form->RecMeetingMsg(msg.sender_accid_, msg.attach_);
						}
						return;
					}
				}
