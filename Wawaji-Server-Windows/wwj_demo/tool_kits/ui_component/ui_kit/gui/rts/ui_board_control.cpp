...
void DrawSinglePen::RenderSelf(HDC hdc, HBITMAP bitmap, bool is_continue)
{
	...
	{
		HBRUSH current_brush = CreateSolidBrush(color_ | 0xff000000);
...
void DrawSinglePen::DrawSingleLine(Gdiplus::Graphics &graphics, Gdiplus::Brush &brush, Gdiplus::Pen &pen, const ui::CPoint &pt_start, const ui::CPoint &pt_end)
{
	...
	graphics.DrawLine(&pen, Gdiplus::Point(start_x, start_y), Gdiplus::Point(end_x, end_y));
	graphics.FillEllipse(&brush, GetRectByPt(pt_end));
}
Gdiplus::RectF DrawSinglePen::GetRectByPt(const ui::CPoint& pt)
{
	int x = pt.x;
	int y = pt.y;
	MeasurePoint(x, y);
	return Gdiplus::RectF(x - line_width_ * 1.0 / 2, y - line_width_ * 1.0 / 2, line_width_, line_width_);
}
