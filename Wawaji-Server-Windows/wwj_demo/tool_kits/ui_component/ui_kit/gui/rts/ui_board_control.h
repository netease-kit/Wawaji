...
public:
	...
	*/
	DrawSinglePen(int x, int y, const RECT& rc_valid)
		...
		draw_pos_(0), layer_(false), smoothing_mode_(false)
	{
	...
	/**
	* 获取轨迹
	* @return std::vector<ui::CPoint> 返回轨迹列表
	*/
	std::vector<ui::CPoint>	GetPtList() { return m_vtPoints; }
	...
	* @return void	无返回值
	*/
	void SetLayerMode(bool layer) { layer_ = layer; }

	/**
	* 设置圆滑轨迹
	* @return void	无返回值
	*/
	void SetSmoothingMode(bool smoothing_mode) { smoothing_mode_ = smoothing_mode; }
protected:
	/**
	* 把本绘制单元的内容绘制到目标DC
	* @param[in] hdc 目标DC
...
private:
	...
	* 根据一个坐标获得一个范围，用于绘制线帽
	* @param[in] pt 坐标
	* @return Gdiplus::RectF	线帽范围
	*/
	Gdiplus::RectF GetRectByPt(const ui::CPoint& pt);
private:
	std::vector<ui::CPoint>	m_vtPoints;
	int draw_pos_;	//绘制度（对于复杂的一步操作设置完成度，不需要每次都重新画）
	bool layer_;
	bool smoothing_mode_; //圆滑轨迹
};
